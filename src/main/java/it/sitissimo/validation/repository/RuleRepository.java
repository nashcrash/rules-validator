...
public interface RuleRepository extends JpaRepository<Rule, Long> {
    ...
    @Query(value = "select distinct rule from Rule rule left join fetch rule.params",
        countQuery = "select count(distinct rule) from Rule rule")
    Page<Rule> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct rule from Rule rule left join fetch rule.params")
    List<Rule> findAllWithEagerRelationships();

    @Query("select rule from Rule rule left join fetch rule.params where rule.id =:id")
