...
@Component(GenericOperator.Names.existsLesserThanOperator)
public class ExistsLessOperator extends AbstractOperator implements GenericOperator {
    ...
    public List<RvValidationResultDetailDTO> validate(Object[] params, JsonNode jsonNode, RvRuleDTO ruleDTO) throws ValidationException {
        ...
        boolean valid = false;
        int found = checkRuleOnCollection(rvRuleDTO, collection, ref);
        if (found < ref) {
            valid = true;
        }
        ...
        if (valid) {
            ...
            return makeRvValidationResultDetailDTO(GenericOperator.Names.existsLesserThanOperator, params, jsonNode, ruleDTO);
        }
    }
}
