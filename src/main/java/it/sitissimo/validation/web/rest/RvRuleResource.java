...
public class RvRuleResource {
    ...
    public ResponseEntity<RvRuleDTO> createRvRule(@Valid @RequestBody RvRuleDTO rvRuleDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/rv-rules/" + result.getId()))
     ...
     */
    @GetMapping("/rv-rules")
    public ResponseEntity<List<RvRuleDTO>> getAllRvRules(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
